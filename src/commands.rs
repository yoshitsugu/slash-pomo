...
pub fn init_pomo(user_id: String) -> redis::RedisResult<()> {
    ...
    let _ : () = try!(con.set(key.to_string(), serde_json::to_string(&score).unwrap()));
    Ok(())
}
...
pub fn get_or_create_pomo(user_id: String) -> redis::RedisResult<PomoScore> {
    let con = try!(redis_con());
    let key = get_key(&user_id);
    let exists: bool = try!(con.exists(key.to_string()));;
    if !exists {
        let _ = init_pomo(user_id);
    }
    let score_json: String = try!(con.get(key.to_string()));
...
pub fn done_pomo(user_id: String) -> redis::RedisResult<PomoScore> {
    ...
    if score.remaining > 0 {
        ...
        let _ : () = try!(con.set(key.to_string(), serde_json::to_string(&new_score).unwrap()));
        Ok(new_score)
    } else {
        Ok(score)
    }
