...
impl PomoScore {
    ...
    fn show_histories(&self) -> String {
        ...
        for h in self.histories.clone() {
            let _p = if h.point.len() > 0 {
                h.point.clone()
            } else {
                "-".to_string()
            };
            let _c = if h.comment.len() > 0 {
                h.comment
            } else {
                "--".to_string()
            };
        ...
        let average = if real_len == 0 {
            0.0
        } else {
            (((sum as f32 / real_len as f32) * 10.0).round() / 10.0)
        };
        hst = format!("{}\n===\naverage: {}", hst, average);
        hst
    }
